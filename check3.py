...
class FaceRecognition:
    ...
    def run_recognition(self):
        ...
        if not cap4.isOpened():
            print("Error: Could not open video stream from IP camera 4.")
            exit()
        ...
        while True:
            if ret1 and ret2 and ret3 and ret4:
                ...
                for (top, right, bottom, left), name in zip(self.face_locations, self.face_names):
                    ...
                    if name.startswith('nguyen'):
                        cv2.putText(combined_frame_resized, "Nguyen Duong", (left + 6, bottom - 6), cv2.FONT_HERSHEY_DUPLEX, 0.8, (255, 255, 255), 1)
                    else:
